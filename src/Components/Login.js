...
signInWithEmailAndPassword(auth, Email.current.value,password.current.value)
  ...
  return (
    <div>
      ...
      <form onSubmit={(e)=>e.preventDefault()} className='absolute w-3/12 bg-opacity-90 bg-black p-8 m-8 my-20 mx-auto right-0 left-0 text-white bg-blend-lighten'>
      <h1 className='text-white font-bold text-2xl py-4 px-2'>{issignin?'Sign Up':'Log In'}</h1>
      {issignin && (<input type='text' ref={Name} placeholder='Name' className='p-2 m-2 w-11/12 bg-gray-700 rounded-md'/>)}
        <input type='text' ref={Email} placeholder='Email' className='p-2 m-2 w-11/12 bg-gray-700 rounded-md'/>
        <input  type='password' ref={password} placeholder='password' className='p-2 m-2 w-11/12 rounded-md  bg-gray-700'/>
        <br></br>
        <p className='text-red-500 font-bold'>{errorstore}</p>
        <button onClick={()=>handlebuttonchange()} className='bg-red-700 p-2 m-2 w-11/12 rounded-md'>{issignin?'Sign Up':'Log In'}</button>
